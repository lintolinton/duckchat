...
#define REQ_SERVER_JOIN 8 /*S2S JOIN */
#define REQ_SERVER_LEAVE 9 /*S2S JOIN */
#define REQ_SERVER_SAY 10 /*S2S JOIN */
...
struct text_server_say{
    request_t txt_type;
    uint64_t txt_server_id;
    char txt_server_channel[CHANNEL_MAX];
    char txt_server_username[USERNAME_MAX];
    char txt_server_text[SAY_MAX];
} packed;
